<template>
  <el-dialog width="90%" :visible.sync="addPlanDialog" title="添加胶料日生产计划">
    ...
    <el-table :data="plansForAdd" border>
      <el-table-column prop="equip_.equip_no" label="机台" width="150"></el-table-column>
      ...
      <el-table-column label="早班计划">
        <el-table-column label="顺序" width="210">
          <template
            ...
            slot-scope="scope"
          >
            <el-input-number v-model.number="scope.row.pdp_product_classes_plan[0].sn" :min="0"></el-input-number>
          </template>
        </el-table-column>
      ...
      <el-table-column label="中班计划">
        <el-table-column label="顺序" width="210">
          <template
            ...
            slot-scope="scope"
          >
            <el-input-number v-model.number="scope.row.pdp_product_classes_plan[1].sn" :min="0"></el-input-number>
          </template>
        </el-table-column>
        <el-table-column label="车次" width="210">
      ...
      <el-table-column label="晚班计划">
        <el-table-column label="顺序" width="210">
          <template
            ...
            slot-scope="scope"
          >
            <el-input-number v-model.number="scope.row.pdp_product_classes_plan[2].sn" :min="0"></el-input-number>
          </template>
        </el-table-column>
        <el-table-column label="车次" width="210">
...
export default {
  ...
  methods: {
    ...
    async getRubberMateria() {
      try {
        let rubberMateriaData = await getRubberMateria({
          all: 1,
          used_type: 4,
        });
    ...
    async getPlanSchedules() {
      try {
        let planSchedulesData = await getPlanSchedules({
          all: 1,
