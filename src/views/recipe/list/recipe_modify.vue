...
<script>
import { recipe_list, rubber_process_url, raw_material_url, material_type_url, condition_url, action_url } from "@/api/recipe_fun";
...
export default {
  ...
  methods: {
    ...
    async popup_material_list(id) {
        try {
        let popup_materialData = await recipe_list("get", id );
    ...
    async put_recipe_list(id, obj) {
        try {
                let recipe_listData = await recipe_list("put", id,obj);
        } catch (e) {}
        },
    async put_recipe_info_step_list(id, obj) {
        try {
                let recipe_info_step_list = await rubber_process_url("put", id, obj);
        } catch (e) {}
        },
